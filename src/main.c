...
void ensure_chunks_worker(Player *player, Worker *worker) {
    ...
    for (int dp = -r; dp <= r; dp++) {
        for (int dq = -r; dq <= r; dq++) {
            ...
            int invisible = !chunk_visible(planes, a, b, Y_BOTTOM_1, Y_TOP_1);
            int priority = 0;
            if (chunk) {
...
void paste() {
    ...
    for (int y = Y_BOTTOM_1; y < Y_TOP_1; y++) {
        for (int x = 0; x <= dx; x++) {
            for (int z = 0; z <= dz; z++) {
...
void on_light() {
    ...
    int hx, hy, hz;
    int hw = hit_test(0, s->x, s->y, s->z, s->rx, s->ry, &hx, &hy, &hz);
    if (hy > Y_BOTTOM_1 && hy < Y_TOP_1 && is_destructable(hw)) {
...
void on_right_click() {
    ...
    if (hy > Y_BOTTOM_1 && hy < Y_TOP_1 && is_obstacle(hw)) {
